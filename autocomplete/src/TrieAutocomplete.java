...
public class TrieAutocomplete implements Autocompletor {
	...
	private void add(String word, double weight) {
		...
		if(current.myWeight > weight){
			while(!parent.equals(myRoot)){
				double max = 0;
				for(Node child : parent.children.values()){
				if(child.myWeight > max)
					max = child.myWeight;
				}
				parent.mySubtreeMaxWeight = max;
	...
	public String[] topKMatches(String prefix, int k) {
		...
		PriorityQueue<Node> pq = new PriorityQueue<Node>(new Node.ReverseSubtreeMaxWeightComparator()); //a node max-heap sorted by mySubtreeMaxWeight
		PriorityQueue<Node> words = new PriorityQueue<Node>(); //a collection of words
		...
		pq.add(current);
		while(!pq.isEmpty()){
			if(words.size()==k && words.peek().getWeight() > pq.peek().mySubtreeMaxWeight)
				break;
			current = pq.poll();
			if(current.isWord)
				if(words.size()<k)
